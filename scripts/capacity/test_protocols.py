...
class ProtocolTests(unittest.TestCase):
    ...
    def test_bounded_payloads_and_entry_paths(self):
        for protocol in PROTOCOLS:
            self.assertIn(path_for(protocol),['/v1/chat/completions','/nt/v1/chat/completions','/v1/messages','/v1/responses'])
            body=request_body(protocol,False,20)
            self.assertIn('x'*20,json.dumps(body))
            if protocol=='node':
                self.assertEqual(body['model'],'gemma3:270m')
                self.assertNotIn('node:',body['model'])
    ...
    def test_nginx_has_protocol_specific_limits_without_unbounded_defaults(self):
        ...
        for path,limit in [('/v1/chat/completions','96m'),('/nt/v1/chat/completions','96m'),('/v1/messages','32m'),('/v1/responses','80m'),('/node/v1/','2m')]:
            match=re.search(r'location\s+(?:=\s+|\^~\s+)?'+re.escape(path)+r'\s*\{([^}]*)\}',config)
