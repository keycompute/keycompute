...
mod tests {
    ...
    fn golden_fixtures_round_trip_without_projection() {
        for fixture in [
            include_str!("../fixtures/node-native-chat-v1.json"),
...
mod chat_regression_tests {
    ...
    fn rejects_limits_headers_stream_and_invalid_usage() {
        ...
        request.validate("m").unwrap();
        let requirements =
            crate::node_capability::NativeRequirements::from_request(&request).unwrap();
        assert!(
            !crate::node_capability::NativeModelProfile::plain_chat("m").permits(&requirements)
        );
