...
WITH matched AS NOT MATERIALIZED (
    ...
    FROM usage_logs ul JOIN page p ON p.user_id = ul.user_id GROUP BY ul.user_id
), earnings AS (
    SELECT ul.user_id, SUM(dr.share_amount) AS amount
    FROM page p JOIN usage_logs ul ON ul.user_id = p.user_id
    JOIN distribution_records dr ON dr.usage_log_id = ul.id
    WHERE dr.beneficiary_id = $1 GROUP BY ul.user_id
...
/// actual relationship grants visibility, not an arbitrary tenant filter.
